...
void enPhysics::ApplyTorque(const enVector3 &torque)
{
	body->applyTorque(btVector3(torque.x, torque.y, torque.z));	
}

void enPhysics::ApplyCentralForce(const enVector3 &force)
{
	body->applyCentralForce(btVector3(force.x, force.y, force.z));	
}

enVector3 enPhysics::GetAngularVelocity()
{
	return enVector3(body->getAngularVelocity().m_floats);
}
void enPhysics::SetAngularVelocity(const enVector3 &velocity)
{
	body->setAngularVelocity(btVector3(velocity.x, velocity.y, velocity.z));	
}

enVector3 enPhysics::GetPosition()
{
	return enVector3(body->getCenterOfMassPosition().m_floats);
}
...
void enPhysics::SetRotation(enScalar yaw, enScalar pitch, enScalar roll)
{
	...
	body->getWorldTransform().setRotation(btQuaternion(DEG2RAD(yaw), DEG2RAD(pitch), DEG2RAD(roll)));
...
void enPhysics::Step()
{
	...
	transform.rotation.angle = RAD2DEG(rot.getAngle());
